...
export default defineEventHandler(async (event) => {
    ...
    try {
        const url = `${config?.api_endpoint}/services/data/v${config?.api_version}/actions/standard/initiateRenewal`;
            ...
            try {
                ...
                response.data.foeEach(async(res) => {
                    if (res?.outputValues?.renewRecordId) {
                        const activeUrl = `${config?.api_endpoint}/services/data/v${config?.api_version}/sobjects/Order/${res?.outputValues?.renewRecordId}`;
